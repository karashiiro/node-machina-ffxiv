...
module.exports = async (struct) => {
    ...
    for (let i = 0; i < 4; i++) {
		struct.statusEntries.push({
			index: struct.data[0x18 + i + (STATUS_EFFECT_ENTRY_LENGTH * i)],
			unknown3: struct.data[0x19 + i + (STATUS_EFFECT_ENTRY_LENGTH * i)],
			id: MachinaModels.getUint16(struct.data, 0x1A + i + (STATUS_EFFECT_ENTRY_LENGTH * i)),
			param: MachinaModels.getUint16(struct.data, 0x1C + i + (STATUS_EFFECT_ENTRY_LENGTH * i)),
			duration: MachinaModels.getFloat(struct.data, 0x20 + i + (STATUS_EFFECT_ENTRY_LENGTH * i)),
			sourceActorID: MachinaModels.getUint32(struct.data, 0x24 + i + (STATUS_EFFECT_ENTRY_LENGTH * i)),
		});
	}
};
